...
const followupNav: NavItem[] = [
  ...
  { href: "/dashboard/followup/leads", label: "Assigned Leads", icon: FileText },
];
...
const adminNav: NavItem[] = [
  ...
  { href: "/dashboard/admin/followups", label: "Follow-Up Team", icon: UserCheck },
];
  ...
  const sidebarContent = (
    ...
    <div className="p-4 md:hidden">
      ...
      <div className="flex items-center gap-3 px-6 py-5 border-b border-harvest-100">
        <img src="/applogo.jpg" alt="The Harvest Logo" className="w-auto h-20" />
        <div className="hidden sm:block">
          <h1 className="font-display font-bold text-earth-900 text-lg leading-tight">The Harvest</h1>
        </div>
      </div>
      ...
      <nav className="flex-1 p-4 space-y-1">
        {navItems.map(item => {
          const active = pathname === item.href || (item.href !== "/dashboard/admin" && item.href !== "/dashboard/evangelist" && item.href !== "/dashboard/followup" && pathname.startsWith(item.href));
          return (
            <Link
              key={item.href}
              href={item.href}
            ...
            >
              ...
              {active && <ChevronRight className="w-3.5 h-3.5 ml-auto opacity-70 hidden sm:block" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="p-4 border-t border-harvest-100">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-harvest-50 group">
          ...
          <div className="flex-1 min-w-0 sm:block">
            ...
            <span className={cn("text-xs font-semibold px-1.5 py-0.5 rounded-md", ROLE_COLORS[role])}>
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-40 p-2 rounded-xl hover:bg-harvest-50 text-earth-700"
      >
